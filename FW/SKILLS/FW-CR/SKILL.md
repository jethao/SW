---
name: fw-cr
description: Review AirHealth firmware pull requests by inspecting open PRs in the `jethao/SW` repository that affect `FW`, checking the implementation against the linked Linear ticket in the `FIR` team, requesting changes when the code or PR does not accurately satisfy the ticket, and approving the PR when the implementation and ticket alignment both look correct.
---

# Firmware Code Review

Use this skill when Codex should act as the firmware reviewer for AirHealth pull requests.

The purpose of this skill is to review open firmware PRs, verify that each PR correctly implements the linked Linear ticket, and take the appropriate review action:
...
- firmware area of interest: `FW`
- Linear team: `FIR`
- Linear team view reference: `https://linear.app/airhealth/team/FIR/all`

Prefer PRs that change files under:

- `FW/`
- especially `FW/Source`

If a PR is open against the SW repo but does not materially touch firmware scope, skip it.
...
- changed files under `SW/FW` or `FW`
...
- Linear issue key such as `FIR-123` in the PR title
- Linear issue key such as `FIR-123` in the branch name
...
- touches the right firmware modules
...
- protocol mismatch
- persistence or replay issues
- low-power, OTA, or factory-mode safety issues where relevant
...
- protocol validation
- HIL or bench evidence when the ticket calls for it
...
- code changes are coherent and firmware-appropriate
